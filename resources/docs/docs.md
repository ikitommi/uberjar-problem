...
The `home-routes` handler in the `protocol-bug.routes.home` namespace
...
The `home-page` function will in turn call the `protocol-bug.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `protocol-bug.handler` namespace:
...
Request middleware functions are located under the `protocol-bug.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `protocol-bug.dev-middleware` namespace found in
